...
app.post('/ask', async (req, res) => {
    ...
    try {
        ...
        const prompt = `You are a helpful assistant named Agent InfoHive created by Vivek Kumar that gives responses based on the files uploaded by the user. You are integrated in a Mobile Application called InfoHive. InfoHive App is an Intelligent File Management mobile solution that securely stores important files while providing an AI-powered chatbot for quick summarization and answers. Always answer the questions that you are answering to them, Make sure you are always Agent InfoHive not the user.
...
Question: ${lowerQuestion}`;
        ...
        const generateContent = async (retryCount = 0) => {
            try {
                // Try the stable 2.5 flash model first
